  ...
  try {
    if (req.method === 'POST') {
      ...
      const [block0, block1] = await Promise.all([
        axios.get(
          `https://api.etherscan.io/api?module=block&action=getblocknobytime&timestamp=${start}&closest=before&apikey=${ETHERSCAN_KEY}`
        ),
        axios.get(
          `https://api.etherscan.io/api?module=block&action=getblocknobytime&timestamp=${end}&closest=before&apikey=${ETHERSCAN_KEY}`
        )
      ]);
      ...
      const transfers = await Promise.all(
        filterQuery.map(async (t) => {
          const transferFrom = t.args.from.toLowerCase();
          const transferTo = t.args.to.toLowerCase();
          let type;

          if (
            transferFrom !== tokenAddress &&
            transferFrom !== pairAddress &&
            transferFrom !== burn &&
            transferTo === pairAddress
          ) {
            type = 'Sell';
          }

          if (
            transferTo !== tokenAddress &&
            transferTo !== pairAddress &&
            transferTo !== burn &&
            transferFrom === pairAddress
          ) {
